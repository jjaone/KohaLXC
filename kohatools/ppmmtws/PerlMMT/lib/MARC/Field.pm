...
sub addSubfield {
    ...
    if (ref $subfield_code eq 'MARC::Subfield') {
        $sf = $subfield_code;
    }
    else {
        my $subfield_content = $_[2];
        $sf = MARC::Subfield->new($subfield_code, $subfield_content);
    }
...
sub deleteSubfield {
    ...
    return 0 if (ref $subfield ne 'MARC::Subfield');
...
sub mergeField {
    ...
    unless (ref $newField eq 'MARC::Field') {
    ...
    if ($newSfs) {
        foreach my $sf (@$newSfs) {
            ...
            my $sfCopy = MARC::Subfield->new($sf->code(), $sf->content());
...
sub mergeAllSubfields {
    ...
    unless (ref $targetSubfield eq 'MARC::Subfield') {
    ...
    foreach my $k (keys %$self) {
        ...
        if (ref $self->{$k} eq "ARRAY") {
            ...
            for(my $i=0 ; $i < @{$array} ; $i++) {
                if (ref $array->[$i] eq "MARC::Subfield") {
                    $array->[$i]->DESTROY();
                }
                undef $array->[$i];
            }
