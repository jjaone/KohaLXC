package Repository::AuthoritiesRepository;
...
$param4/totalColumns the amount of columns expected for one row. If this amount is not reached, we can suspect a broken row and try to fix it.
...
sub fetch {
    ...
    if (! defined $retval) {
        $self->{logger}->warning('No authority for pk = '.$pk.(($key) ? ', key = '.$key : '')) unless $noWarnings;
        return undef;
    }
    else {
        return $retval;
